...
namespace WamWooWam.Core.Windows
{
    ...
    public class Glass
    {
        ...
        public static void ExtendGlassFrame(Window window, Thickness margin)
        {
            try
            {
                ...
                // Get System Dpi  
                using (var desktop = Graphics.FromHwnd(mainWindowPtr))
                {
                    float dpiX = desktop.DpiX;
                    float dpiY = desktop.DpiY;

                    var margins = new MARGINS(margin, dpiX, dpiY);
    ...
    public class Thumbnails
    {
        ...
        {
            var hBitmap = GetHBitmapThumbnail(Path.GetFullPath(fileName), width, height, options);
